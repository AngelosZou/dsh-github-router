...
export async function aggregateIssue(ctx, options, args) {
  ...
  const state = {
    ...
    fetched: { meta: false, comments: false },
  }
  ...
  if (state.issue !== null) {
    try {
      ...
      state.fetched.comments = true
    } catch (error) {
      state.notes.push(`api comments: ${String(error && error.message ? error.message : error).slice(0, 180)}`)
    }
  }

  if (options.routes.gh && subprocess !== undefined && (state.issue === null || !state.fetched.comments)) {
    let ghUsable = false
    try {
    ...
    if (ghUsable) {
      try {
        ...
        if (!state.fetched.comments && Array.isArray(g.comments)) {
          state.comments = capItems(
            g.comments.map((c) => ({
              kind: 'comment',
  ...
  if (options.routes.html && (state.issue === null || !state.fetched.comments)) {
    try {
      await attemptLadder(
        async (proxy) => {
          ...
          if (!state.fetched.comments && parsed.discussion.length > 0) {
