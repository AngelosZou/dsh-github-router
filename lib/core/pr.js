...
export async function aggregatePr(ctx, options, args) {
  ...
  const state = {
    ...
    notes: [],
    // Which parts a route actually served. A successfully fetched empty
    // list is authoritative and must NOT trigger a fallback ladder — the
    // routes are failover, not fan-out (docs/design.md).
    fetched: { meta: false, discussion: false, reviews: false, commits: false, files: false, diff: false },
  }
  ...
  const needGh =
    state.pr === null ||
    (include.files && !state.fetched.files) ||
    (include.commits && !state.fetched.commits) ||
    (include.reviews && !state.fetched.reviews) ||
    (include.discussion && !state.fetched.discussion)
  if (options.routes.gh && subprocess !== undefined && needGh) {
    ...
    if (ghUsable) {
      ...
      if (include.diff && !state.fetched.diff) {
        try {
  ...
  // ---------------------------------------------------------- 3) html route
  if (options.routes.html && (state.pr === null || (include.discussion && !state.fetched.discussion))) {
    try {
      await attemptLadder(
        async (proxy) => {
          ...
          if (include.discussion && !state.fetched.discussion && parsed.discussion.length > 0) {
  ...
  // ------------------------------------------------------------ 4) git route
  if (options.routes.git && subprocess !== undefined && ((include.commits && !state.fetched.commits) || (include.diff && !state.fetched.diff))) {
    let gitUsable = false
    try {
    ...
    if (gitUsable) {
      try {
        ...
        if (read !== null && read !== undefined) {
          if (include.commits && !state.fetched.commits && read.commits.length > 0) {
            state.commits = capItems(read.commits, maxItems, state.notes, 'commits')
            state.routes.commits = 'git'
            state.fetched.commits = true
          }
          if (include.diff && !state.fetched.diff && read.diff !== null) {
