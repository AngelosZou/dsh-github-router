...
const PR_FIELDS =
  ...
  'mergeable,reviewDecision,reviews,comments,commits,files,url'
const ISSUE_FIELDS = 'author,title,body,state,stateReason,createdAt,updatedAt,closedAt,comments,labels,assignees,url'
 ...
 *
 * The number is positional and the repository is passed with `--repo`:
 * `gh pr view` also accepts a branch, so an `owner/repo/N` positional would
 * be resolved as a branch against the local git repository instead.
 */
export async function ghPrView(subprocess, owner, repo, number, spec = {}) {
  return ghJson(subprocess, ['gh', 'pr', 'view', String(number), '--repo', `${owner}/${repo}`, '--json', PR_FIELDS], spec)
}

/** `gh pr diff <number> --repo <owner/repo>` — capped text of the PR diff. */
export async function ghPrDiff(subprocess, owner, repo, number, spec = {}) {
  const out = await runCaptured(subprocess, {
    argv: ['gh', 'pr', 'diff', String(number), '--repo', `${owner}/${repo}`],
    graceMs: 30_000,
    signal: spec.signal,
    env: childEnv(),
    maxStdout: spec.maxStdout ?? 1048576,
 ...
 *
 * `gh issue view` accepts only a bare number or a URL, so the repository
 * must travel in `--repo` — never as part of the positional argument.
 */
export async function ghIssueView(subprocess, owner, repo, number, spec = {}) {
  return ghJson(subprocess, ['gh', 'issue', 'view', String(number), '--repo', `${owner}/${repo}`, '--json', ISSUE_FIELDS], spec)
}
