...
function isPrLike(node) {
  return (
    ...
    ('additions' in node ||
      'headRefName' in node ||
      'headRef' in node ||
      // current page payload: baseBranch/headBranch are plain strings
      'headBranch' in node ||
      'baseBranch' in node)
  )
}
...
/**
 * First non-empty string among the candidates, else null. GitHub renames
 * payload fields between page generations (createdAt/createdTime,
 * headRefName/headBranch, mergedAt/mergedTime), so every field maps from a
 * small ordered list of spellings.
 */
function str(...values) {
...
export function prFromPayloads(objects) {
  ...
  return {
    ...
    createdAt: str(pr.createdAt, pr.createdTime),
    updatedAt: str(pr.updatedAt, pr.updatedTime),
    mergedAt: str(pr.mergedAt, pr.merged_at, pr.mergedTime),
    baseRef: str(pr.baseRefName, pr.baseBranch) ?? refName(pr.baseRef ?? pr.base),
    headRef: str(pr.headRefName, pr.headBranch) ?? refName(pr.headRef ?? pr.head),
    baseSha: str(pr.baseRefOid, pr.baseSha) ?? shaOf(pr.baseRef ?? pr.base),
    headSha: str(pr.headRefOid, pr.headSha) ?? shaOf(pr.headRef ?? pr.head),
