...
/** True when a parsed response head declares chunked transfer encoding. */
export function isChunked(headers) {
  const value = headers === null || headers === undefined ? '' : headers['transfer-encoding']
  return typeof value === 'string' && /(?:^|[\s,])chunked(?:[\s,;]|$)/i.test(value)
}

/**
 * Incremental `Transfer-Encoding: chunked` decoder.
 *
 * The tunnel reads the raw TLS socket, so the framing has to be stripped
 * here: otherwise a proxied response keeps its size lines and trailers
 * embedded in the body (GitHub serves issue/PR pages that way), which
 * corrupts every JSON island and file read on that path.
 *
 * `onData(chunk)` receives decoded bytes and returns true to stop early
 * (byte cap reached). `onDone()` fires after the terminating zero-length
 * chunk and its trailer section; `onError(error)` on a malformed or
 * truncated stream. `end()` must be called when the socket closes.
 */
export function createChunkedDecoder(onData, onDone, onError) {
  let state = 'size' // size | data | dataEnd | trailer | done
  let remaining = 0
  ...
  return {
    push(buf) {
      if (state === 'done' || buf.length === 0) return
      ...
      while (state !== 'done') {
        if (state === 'size') {
          ...
          const line = pending.subarray(0, idx).toString('latin1')
          pending = pending.subarray(idx + 2)
          const size = line.split(';')[0].trim() // chunk extensions are legal
          if (!/^[0-9a-fA-F]+$/.test(size)) {
            fail(`invalid chunk size: ${line.slice(0, 40)}`)
            return
          }
        ...
        if (state === 'data') {
          ...
          remaining -= take
          if (onData(piece) === true) {
            state = 'done'
            return
          }
          if (remaining === 0) state = 'dataEnd'
          continue
        }
        if (state === 'dataEnd') {
          if (pending.length < 2) return
          if (pending[0] !== 0x0d || pending[1] !== 0x0a) {
            fail('malformed chunked body: missing CRLF after chunk data')
            return
          }
        ...
        const idx = pending.indexOf('\r\n')
        if (idx === -1) {
          if (pending.length > 65536) fail('chunk trailer too large')
          return
        }
        const line = pending.subarray(0, idx).toString('latin1')
        pending = pending.subarray(idx + 2)
      ...
      fail('connection closed before the chunked body completed')
    },
  }
}
...
function readResponse(socket, maxBytes) {
  return new Promise((resolve, reject) => {
    ...
    const feed = (buf) => {
      ...
      if (decoder !== null) decoder.push(buf)
      else onBody(buf)
    }
    socket.on('data', (buf) => {
      if (!headerDone) {
        ...
        if (isChunked(headers)) {
          decoder = createChunkedDecoder(
            (piece) => onBody(piece),
            () => finish(),
            (error) => finish(error),
          )
        }
        feed(headerBuf.subarray(idx + 4))
        return
      }
      feed(buf)
    })
    socket.on('end', () => {
      if (!headerDone) {
        finish(new Error('connection closed before response headers'))
        return
      }
      if (decoder !== null) {
        decoder.end()
